...
# === Создание инбаундов для каждого домена ===
SUCCESS_COUNT=0
FAILED_COUNT=0
SERVER_IP=$(curl -s --max-time 3 https://api.ipify.org || curl -s --max-time 3 https://4.ident.me)

# Переменные для сохранения данных первого успешного инбаунда (для примера ссылки)
FIRST_SUCCESS_DOMAIN=""
FIRST_SUCCESS_UUID=""
FIRST_SUCCESS_EMAIL=""
FIRST_SUCCESS_SHORT_ID=""

echo -e "${green}Создание инбаундов для каждого домена (всего: ${#DOMAINS[@]})...${plain}" >&3
...
for domain in "${DOMAINS[@]}"; do
    ...
    # Формируем безопасное имя для remark (заменяем точки на подчеркивания)
    SAFE_DOMAIN=$(echo "$domain" | sed 's/\./_/g')
    REMARK="reality443-${SAFE_DOMAIN}"
    ...
    # Формирование JSON для stream settings
    ...
    if echo "$ADD_RESULT" | grep -q '"success":true'; then
        ...
        if [[ $((SUCCESS_COUNT % 10)) -eq 0 ]] || [[ $SUCCESS_COUNT -eq 1 ]]; then
            echo -e "  ${green}✓${plain} Создан инбаунд для ${domain} (${SUCCESS_COUNT}/${#DOMAINS[@]})" >&3
        fi
    else
...
if [[ $SUCCESS_COUNT -gt 0 ]]; then
    ...
    # Создаем ссылку на основе первого успешно созданного инбаунда
    if [[ -n "$FIRST_SUCCESS_DOMAIN" ]]; then
        VLESS_LINK="vless://${FIRST_SUCCESS_UUID}@${SERVER_IP}:443?type=tcp&security=reality&encryption=none&flow=xtls-rprx-vision&sni=${FIRST_SUCCESS_DOMAIN}&fp=chrome&pbk=${PUBLIC_KEY}&sid=${FIRST_SUCCESS_SHORT_ID}&spx=%2F#${FIRST_SUCCESS_EMAIL}"
        ...
        echo -e ""
        echo -e "${yellow}Примечание: Для получения ссылок для других доменов используйте панель управления x-ui.${plain}" >&3
        echo -e ""
        ...
        echo "Все инбаунды используют порт 443 с разными SNI."
        echo ""
        ...
        qrencode -t ANSIUTF8 "$VLESS_LINK"
        echo ""
        echo "Примечание: Для получения ссылок для других доменов используйте панель управления x-ui."
        echo ""
